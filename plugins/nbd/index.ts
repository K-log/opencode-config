...
export default {
  ...
  async server(ctx, _options) {
    ...
    const sessionHooks = sessionLoggerHooks()
    const costHooks = costTrackerHooks()
    const autoStage = autoStageHooks()
    const sessionStats = sessionStatsHooks()
    const autoFormat = autoFormatHooks()
    const branchGuard = branchGuardHooks()
    const commitGuard = commitGuardHooks()
    const undoStack = undoStackHooks()
    ...
    const memorySync = memorySyncHooks()
    const contextInjector = contextInjectorHooks()
    const auditLogger = auditLoggerHooks()
    const costCap = costCapHooks(ctx.client)
    const telemetry = telemetryHooks()
    const recentFilesTracker = recentFilesTrackerHooks()
