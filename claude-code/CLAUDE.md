...
- Before executing `rm`, file overwrites, or any destructive bash command, confirm
  with the `AskUserQuestion` tool. Exception: files created in the same session.
...
- Important: If setting AI-TOOL, always set the value to `claude`.
...
- If there is a branch with the name already, use the `AskUserQuestion` tool and provide some alternatives to choose from.
