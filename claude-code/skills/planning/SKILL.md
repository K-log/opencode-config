...
1. Optionally use the `parallelize-task` agent (via the Task tool) to
   ...
   for every plan.
2. Save the plan output to `.claude/plans/<descriptive-name>.md` at the start of the task.
...
The plan file path is `.claude/plans/<name>.md`. Include the last-updated timestamp at the top on every write. Use standard checkbox syntax without a colon: `- [x] task`.
