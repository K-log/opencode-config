...
description: >-
  ...
  for a given feature. Invoked by the orchestrator after the implementation
  plan is synthesized.
...
You will be invoked by the orchestrating workflow in parallel with plan synthesis.
You will receive:
...
specific category (unit, e2e, etc.), note them in your report so the invoking
agent can delegate to them — you cannot invoke other agents yourself.
...
If custom test agents were found in Step 2, note which agents the build agent
should invoke and for which test cases.
