...
2. **Web research** -- For each relevant package identified in step 1, use
   WebSearch and WebFetch to find the official documentation for the exact
   installed version. Target the version directly using versioned URLs
   (e.g. `/v7/`, `/7.x/`), version-specific subdomains (e.g.
   `v7.mantine.dev`), or targeted searches (e.g. `site:v7.mantine.dev`).
   Before extracting any information, confirm the fetched page matches the
   installed version via the URL, title, or navigation. If it does not,
   locate and fetch the correct version first.

3. **Local fallback** -- If web research returns insufficient results for
