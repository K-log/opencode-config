...
4. **Read-Only**: You examine code and run test/build commands, but never edit files. Restrict Bash usage to read-only git inspection commands (`git status`, `git diff`, `git show`, `git log`, `git merge-base`, `git rev-parse`, `git branch`) and test commands. Never stage, commit, install, or mutate anything.
...
3. **Targeted Test Execution**
   - Only run tests when the caller has stated that tests are opted in for
     ...
     - If a test command is denied by permissions or unavailable in this
       environment, report the exact reason (e.g. "test command was denied"
       or "test runner not found") — never report tests as run when they
       were not.
...
List any test or tool commands that could not be run (denied by permissions,
