...
- Input: Jira ticket key/ID as first argument: `$ARGUMENTS`. Never create new tickets.
...
- Propose refinements for existing tickets only:
  ...
  - `Definition of Done`
- Before writing any ticket updates, present proposed edits via `AskUserQuestion` tool per ticket.
