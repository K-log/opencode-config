...
2. If multiple templates exist (multi-template dir), use the `question` tool to
...
non-default branch), **use the `question` tool** to ask the user to specify the
...
- Match the dominant pattern across multiple recent examples, not a single
  outlier.
- If examples conflict or none exist, use the `question` tool to ask the user
