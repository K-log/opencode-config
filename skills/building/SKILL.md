...
2. Use one built-in `build` delegation per implementation attempt. When the
   orchestrator is active, it owns review of the build output and any retry;
   do not introduce alternate implementation agents or custom build files.
3. Utilize the question tool for anything unclear or any unexpected situations. Do not deviate from the plan without confirmation.
